...
export default function MethodCycleDiagram() {
  ...
  return (
    <svg viewBox="0 0 1040 400" role="img" aria-label={L("学习循环图：按域学、提炼不变量、L3 映射、事故归因，四步循环往复", "Learning cycle: by domain, distill invariants, L3 mapping, incident attribution — four steps, looping")} className="diagram diagram-sm">
      <title>{L("学习四步循环", "The four-step cycle")}</title>
      <defs>
      ...
      <text x={520} y={46} textAnchor="middle" className="mn-t2">{L("按域学", "By domain")}</text>
      <text x={520} y={66} textAnchor="middle" className="mn-s">{L("每域 2–3 份经典材料", "2–3 classics per domain")}</text>
      <circle cx={625} cy={190} r={12} className="mn-node" />
      <text x={625} y={194} textAnchor="middle" className="mn-node-t">2</text>
      <text x={655} y={186} className="mn-t2">{L("提炼不变量", "Distill invariants")}</text>
      <text x={655} y={206} className="mn-s">{L("换语言还成立吗？", "does it survive a language switch?")}</text>
      <circle cx={520} cy={295} r={12} className="mn-node" />
      <text x={520} y={299} textAnchor="middle" className="mn-node-t">3</text>
      <text x={520} y={336} textAnchor="middle" className="mn-t2">{L("L3 映射", "L3 mapping")}</text>
      <text x={520} y={356} textAnchor="middle" className="mn-s">{L("新语言只补实现层", "a new language costs only L3")}</text>
      <circle cx={415} cy={190} r={12} className="mn-node" />
      <text x={415} y={194} textAnchor="middle" className="mn-node-t">4</text>
      <text x={385} y={186} textAnchor="end" className="mn-t2">{L("事故归因", "Incident attribution")}</text>
      <text x={385} y={206} textAnchor="end" className="mn-s">{L("故障定位到某条不变量", "locate the violated invariant")}</text>
    </svg>
  );
}
