export default function D2RaceDiagram({ stage = 5 }: { stage?: number }) {
  ...
  return (
    <svg viewBox="0 0 1040 300" role="img" aria-label="竞态图：两个请求同时先读后写，A 的更新被 B 覆盖" className="diagram diagram-sm">
      <title>D2 并发与一致性 · 先检查后写入的竞态</title>
      <defs>
      ...
      <g className={on(5) ? "bld bld-on" : "bld"}>
        ...
        <text x={540} y={24} textAnchor="middle" className="mn-a">竞态窗口</text>
      </g>
      <text x={40} y={64} className="mn-t">请求 A</text>
      ...
      <text x={40} y={144} className="mn-t">请求 B</text>
      ...
      <text x={120} y={292} className="mn-s">A 基于旧值 1 的写入被 B 覆盖 —— lost update</text>
    </svg>
  );
}
